...
static Backbuffer global_backbuffer;
...
int WINAPI WinMain(HINSTANCE instance, HINSTANCE __unused1, LPSTR __unused2, int __unused3)
{
    ...
    if (RegisterClassExA(&windowclass) == 0)
        assert(!"RegisterClassExA failed");
    ...
    if (!window)
        assert("CreateWindowEx failed");

    ShowWindow(window, SW_SHOW);
    UpdateWindow(window);

    // We'll allocate a backbuffer that has enough room for the maximum
    // possible window size, so we don't have to realloc on windows size changes
